...
class PPO:
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:
            ...
            if i_so_far % self.save_freq == 0:
                torch.save(self.actor.state_dict(),  './ppo_actor_exp2.pth')
                torch.save(self.critic.state_dict(), './ppo_critic_exp2.pth')  
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            for ep_t in range(self.max_timesteps_per_episode):    
                ...
                batch_obs.append(torch.clone(prev_m))    #prev_m 
                batch_mask_vec.append(mask_vec)
                batch_h.append(h)
                batch_c.append(c)
                daction, caction, log_probd, log_probc, h1, c1 = self.get_action(prev_m, mask_vec, h, c)  #prev_m
                val = self.critic(prev_m, h, c)          #prev_m
                h = h1
                c = c1
                ...
                prev = torch.cat((prev_m,obs1),0)        #prev_m
    ...
    def evaluate_d(self, batch_obsd, batch_mask_vec, batch_dacts, batch_h, batch_c):
        ...
        for i in range(int(len(batch_obsd))):
