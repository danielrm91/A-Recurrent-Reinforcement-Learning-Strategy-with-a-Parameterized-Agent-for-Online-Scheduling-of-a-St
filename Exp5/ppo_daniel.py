...
class PPO:
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:
            ...
            self.temperature = self.temp * (1/self.temp)**(i_so_far/t_it)                   # initial * (final/initial)***(t/T)

            #Entropy annealing
            self.ent_coeff = self.ent_init * (0.01/self.ent_init)**(i_so_far/t_it)  # initial * (final/initial)***(t/T)

            #Calling Rollout
            batch_obs, batch_mask_vec, batch_dacts, batch_cacts, batch_log_probsd, batch_log_probsc, batch_lens, batch_vals, batch_rews, done_flags = self.rollout() 
            ...
            if i_so_far % self.save_freq == 0:
                torch.save(self.actor.state_dict(),  './ppo_actor_exp5.pth')
                torch.save(self.critic.state_dict(), './ppo_critic_exp5.pth')
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            for ep_t in range(self.max_timesteps_per_episode):    
                ...
                if prev_m[-1] % 2 == 0:
                    prev_m = prev_m*part_o1

                batch_obs.append(torch.clone(prev_m)) #prev_m
                batch_mask_vec.append(mask_vec)
                daction, caction, log_probd, log_probc = self.get_action(prev_m, mask_vec)  #prev_m
                val = self.critic(prev_m)   #prev_m
