...
class PPO:
    def __init__(self, policy_class, value_class, env, **hyperparameters):
        ...
        self.env = env
        self.obs_dim =      15
        self.sen_dim =      250      #Encoder output
        ...
        #self.cov_var = torch.full(size=(self.out_ac2_dim,),fill_value=0.04)  #fill value: variance, eg variance=0.01, std dev 0.1
        ...
        while t_so_far < total_timesteps:
            ...
            for _ in range(self.n_updates_per_iteration):
                ...
                for start in range (0, b_size, mini_batchsize):  #Making minibatches for updating the networks.
                    ...
                    #approx_kl_c = ((ratioc - 1) - logratios_c).mean()
                    ...
                    #approx_kl_d = ((ratiod - 1) - logratios_d).mean()
                ...
                #if approx_kl_d > self.target_kl or approx_kl_c > self.target_kl:
                #    break 
            ...
            if i_so_far % self.save_freq == 0:
                torch.save(self.actor.state_dict(), './ppo_actor_exp8.pth')
                torch.save(self.critic.state_dict(), './ppo_critic_exp8.pth')  # No 2
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            prev = torch.zeros([self.seq_len,self.obs_size])          # CREATE OBS SEQUENCE
            ...
            for ep_t in range(self.max_timesteps_per_episode):    #+1
                t += 1      
                #
                mu = 1.0                    #average expected
                sigma = 0.03                #standard deviation
                lower, upper = 0.95,1.05    #range of truncation
                up_b,low_b = (lower-mu)/sigma , (upper-mu)/sigma
                v_ = truncnorm.rvs(up_b,low_b,loc=mu,scale=sigma, size=4)     
                part_o1 = np.array([1,1,1,1, v_[0],v_[1],v_[2],v_[3], 1,1,1,1,1,1,1], dtype=np.float32) 
        ...
        # Reshape data as tensors in the appropiate shap        
    ...
    #def compute_rtgs(self, batch_rews):                                    MIGHT ERASE THIS BECAUSE I ALREADY USE GENERALIZED ADVANTAGE FUNCTION
    #    batch_rtgs = []
